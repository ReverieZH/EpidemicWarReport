...
<head>
    ...
    <script>
        ...
        function changeStatus(questionnaireId,Status) {
            ajaxRequest(questionnaireId,Status);
        }

        function ajaxRequest( questionnaireId,status) {
            var ajax;
            ...
            ajax.send("questionnaireId="+questionnaireId+"&status="+status);
        }

        function deleleform(){
            var checkbox = document.getElementsByName('questionnaireId');
            var page=document.getElementById("page").value;
            var url="";
            for (var i = 0; i < checkbox.length; i++) {
                if (checkbox[i].checked) {
                    url+="questionnaireId="+checkbox[i].value+"&";
                }
            }
            url=url.substring(0,url.length-1);
            deleteajaxRequest(url);
            // url+="page="+page;
            // location.href="deleteQuestionnaire.do?"+url+"&taskId"+taskId;
        }
        ...
        function deleteajaxRequest(url) {
            ...
            ajax.open("get", "deleteQuestionnaire.do?"+url);
            ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
            ajax.send();
        }
        ...
        function deleteOneForm(questionnaireId) {
            deletOneajax(questionnaireId)
        }

        function deletOneajax( questionnaireId ) {
            var ajax;
            if(window.XMLHttpRequest){//火狐
                ajax=new XMLHttpRequest();
            }else if(window.ActiveXObject){//ie
            ...
            ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
            ajax.send("questionnaireId="+questionnaireId);
        }
        ...
        function getLink(questionnaireId) {
            layui.use('layer', function(){
                var layer = layui.layer;
                layer.open({
                    type: 2,
                    ...
                    shadeClose: true, //点击遮罩关闭层
                    area : ['800px' , '200px'],
                    content: "getLink.do?questionnaireId="+questionnaireId //这里content是一个普通的String
                });
            })
        }
    </script>
...
<div style="color: #666;">
    ...
    <a style="margin-top: 5px;" href="javascript:history.back()"><i class="layui-icon layui-icon-next" style="color: #666;"></i><cite>表单任务设置</cite></a>
    <a class="layui-btn layui-btn-primary" href="javascript:location.reload()"><i class="layui-icon layui-icon-refresh"></i> </a>
    <button style="float: right;" class="layui-btn layui-btn-primary"  onclick="deleleform()">删除表单</button>
    <button style="float: right;" class="layui-btn layui-btn-primary" onclick="getiframe('getaddQuestionnaire.do?taskId=<%=request.getAttribute("taskId")%>')">添加表单</button>
    <table class="layui-table" style="width: 1050px;" lay-skin="line">
        <tr>
            <td width="2%" height="20" align="center" ><input type="checkbox" onchange="checkAll()" id="check_all_box">全选</td></td>
            <td width="10%" height="20" align="center" >表单信息</td>
        ...
        <%
            Integer i= 0;
            PageBean<Questionnaire> pageBean= (PageBean<Questionnaire>) request.getAttribute("pageBean");
        ...
        <c:forEach begin="0" end="${pageBean.pageSize}" items="${pageBean.list}" var="task">
            <c:if test="${i<pageBean.totalRecord}">
                <tr>
                    <td width="2%" height="50" align="center"><input  type="checkbox" name="questionnaireId" value="<%=pageBean.getList().get(i).getQuestionnaireId()%>"></td>
                    <td width="10%" height="50" align="center">
                        <%= pageBean.getList().get(i).getQueName()%><br>
                        <span style="color: #cccccc">描述:<%=pageBean.getList().get(i).getQueDesc()%></span>
                    </td>
                    <%
                    ...
                    <%}%>
                    <td width="20%" height="50" align="center"><button class="layui-btn" onclick="getiframe('getmodifyQuestionnaire.do?questionnaireId=<%=pageBean.getList().get(i).getQuestionnaireId()%>')">编辑</button> <button class="layui-btn layui-btn-warm" onclick="changeStatus('<%=pageBean.getList().get(i).getQuestionnaireId()%>','0')" >禁用</button><button class="layui-btn layui-btn-danger" onclick="deleteOneForm('<%=pageBean.getList().get(i).getQuestionnaireId()%>')">删除</button><a href="../question/main.do?questionnaireId=<%=pageBean.getList().get(i).getQuestionnaireId()%>" class="layui-btn layui-btn-normal">字段管理</a><button class="layui-btn layui-btn-primary" onclick="getLink('<%=pageBean.getList().get(i).getQuestionnaireId()%>')">生成超链接 </button></td>
                </tr>
            </c:if>
