...
<head>
    ...
    <script>
        ...
        function changeStatus(questionId,Status) {
            ajaxRequest(questionId,Status);
        }

        function ajaxRequest( questionId,status) {
            var ajax;
            ...
            ajax.send("questionId="+questionId+"&status="+status);
        }

        function delelequestion(){
            var checkbox = document.getElementsByName('questionId');
            var url="";
            for (var i = 0; i < checkbox.length; i++) {
                if (checkbox[i].checked) {
                    url+="questionId="+checkbox[i].value+"&";
                }
            }
        ...
        function deleteajaxRequest(url) {
            ...
            ajax.open("get", "deleteQuestion.do?"+url);
            ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
            ajax.send();
        }

        function deleteOneQuestion(questionId) {
            deletOneajax(questionId)
        }

        function deletOneajax( questionId ) {
            var ajax;
            if(window.XMLHttpRequest){//火狐
                ajax=new XMLHttpRequest();
            }else if(window.ActiveXObject){//ie
            ...
            ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
            ajax.send("questionId="+questionId);
        }
...
<div style="color: #666;">
    ...
    <button style="float: right;" class="layui-btn layui-btn-primary"  onclick="delelequestion()">删除字段</button>
    <button style="float: right;" class="layui-btn layui-btn-primary" onclick="getiframe('getaddQuestion.do?questionnaireId=<%=request.getAttribute("questionnaireId")%>')">添加字段</button>
    ...
    <table class="layui-table" style="width: 1050px;" lay-skin="line">
        ...
        <tr>
            ...
            <td width="10%" height="20" align="center" >字段信息</td>
        ...
        <%
            Integer i= 0;
            PageBean<Question> pageBean= (PageBean<Question>) request.getAttribute("pageBean");
        %>
        <c:forEach begin="0" end="${pageBean.pageSize}" items="${pageBean.list}" var="question">
            <c:if test="${i<pageBean.totalRecord}">
                <tr>
                    <td width="2%" height="50" align="center"><input  type="checkbox" name="questionId" value="<%=pageBean.getList().get(i).getQuestionId()%>"></td>
                    <td width="10%" height="50" align="center">
                        <%= pageBean.getList().get(i).getStem()%><br>
                        <span style="color: #cccccc">题目类型:
                            <c:choose>
                                <c:when test="${question.type=='0'}">输入框</c:when>
                                <c:when test="${question.type=='1'}">选择框</c:when>
                                <c:when test="${question.type=='2'}">地址框</c:when>
                                <c:when test="${question.type=='3'}">定位框</c:when>
                            </c:choose>
                    </td>
                    <%
                        if(pageBean.getList().get(i).getValidity().equals("1")){
                    %>
                    <td width="5%" height="50" align="center" style="color: green" >使用中</td>
                    <%}else{%>
                    <td width="5%" height="50" align="center" style="color: red">禁用中</td>
                    <%}%>
                    <td width="12%" height="50" align="center"><button class="layui-btn" onclick="getiframe('getmodifyQuestion.do?questionId=<%=pageBean.getList().get(i).getQuestionId()%>')">编辑</button> <button class="layui-btn layui-btn-warm" onclick="changeStatus('<%=pageBean.getList().get(i).getQuestionId()%>','0')" >禁用</button><button class="layui-btn layui-btn-danger" onclick="deleteOneQuestion('<%=pageBean.getList().get(i).getQuestionId()%>')">删除</button></td>
                </tr>
            </c:if>
