...
<head>
    ...
    <script>
        ...
        function ajaxRequest(applyId,applyName,status) {
            ...
            ajax.open("post", "modifyApply.do");
            ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
            ajax.send("applyId="+applyId+"&applyName="+applyName+"&status="+status);
        }
...
<body>
    ...
    <table class="table_context" width="100%">
        <tr>
            <input type="hidden" name="applyId" id="applyId" value="<%=apply.getApplyId()%>">
            <td width="10%" height="50" align="right">任务名称:<span style="color: red;">*</span></td>
            <td width="10%" height="50" align="left"><input id="applyName" name="applyName"  value="<%=apply.getApplyName()%>" type="text"></td>
        </tr>

        <tr>
            <td width="10%" height="50" align="right">使用状态:</td>
            <td width="10%" height="50" align="left"> <select id="statusop" name="statusop" lay-verify="" >
                <option value="<%=apply.getStatus()%>" disabled selected hidden><%=apply.getStatus().equals("1")?"使用中":"禁用中"%></option>
                <option value="1">启用</option>
                <option value="0">禁用</option>
            </select></td>
        </tr>
        <tr>
            <td height="50" colspan="2" align="center"><button class="layui-btn" onclick="modifyApply()" >保存数据</button><button class="layui-btn layui-btn-danger" onclick="reset()">取消编辑</button></td>
        </tr>
    </table>
</body>
</html>
