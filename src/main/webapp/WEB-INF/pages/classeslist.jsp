...
<html>
	<head>
    	...
    	<script>
                ...
                function deleleMajor(){
                    ...
                    for (var i = 0; i < checkbox.length; i++) {
                        if (checkbox[i].checked) {
                            url+="classesId="+checkbox[i].value+"&";
                        }
                    }
                ...
                function deleteajaxRequest(url) {
                    ...
                    ajax.open("get", "deleteClasses.do?"+url);
                    ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
                    ajax.send();
                }

                function deleteOneClasses(classesId){
                    deletajax(classesId);
                }

                function deletajax(classesId ) {
                    var ajax;
                    if(window.XMLHttpRequest){//火狐
                        ajax=new XMLHttpRequest();
                    }else if(window.ActiveXObject){//ie
	...
	<body style=" overflow-x:hidden;"  class="inner-container">
		<div style="color: #666;">
            ...
            <button style="float: right;" class="layui-btn layui-btn-primary" onclick="deleleMajor()">删除班级</button>
		<button style="float: right;" class="layui-btn layui-btn-primary" onclick="getiframe('getaddClasses.do')">添加班级</button>
		...
		<table class="layui-table" style="width: 1050px;" lay-skin="line">
		             <tr>
                        ...
                        <td width="10%" height="20" align="center" >班级信息</td>
                         <td width="10%" height="20" align="center" >所属专业</td>
            ...
            <%
                Integer i= 0;
                PageBean<Classes> pageBean= (PageBean<Classes>) request.getAttribute("pageBean");
            %>
            ...
            <c:forEach begin="0" end="${pageBean.pageSize}" items="${pageBean.list}" var="task">
                <c:if test="${i<pageBean.totalRecord}">
                    <tr>
                        ...
                        <td width="2%" height="50" align="center"><input  type="checkbox" name="classesId" value="<%=pageBean.getList().get(i).getClassesId()%>"></td>
                        <td width="10%" height="50" align="center">
                            <%= pageBean.getList().get(i).getClassesName()%><br>
                        </td>
                        <td width="10%" height="50" align="center">
                        ...
                        <%}%>
                        <td width="12%" height="50" align="center"><button class="layui-btn" onclick="getiframe('getmodifyClasses.do?classesId=<%=pageBean.getList().get(i).getClassesId()%>')">编辑</button> <button class="layui-btn layui-btn-warm" onclick="changeStatus('<%=pageBean.getList().get(i).getClassesId()%>','0')" >禁用</button> <button class="layui-btn layui-btn-danger" onclick="deleteOneClasses('<%=pageBean.getList().get(i).getClassesId()%>')">删除</button></td>
                    </tr>
                </c:if>
