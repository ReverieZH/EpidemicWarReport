...
<head>
    ...
    <script>
        ...
        function modifyStudent() {
            ...
            ajaxRequest(sno,name,password,sex,grade,type,academicName,majorName,classesName,dormitory,origin,address,phone,status);
            // location.href="task/addTask.do?"+"taskName="+taskName+"&taskDesc="+taskDesc+"&date="+date;


        }

        function ajaxRequest( sno,name,password,sex,grade,type,academicName,majorName,classesName,dormitory,origin,address,phone,status) {
            ...
            ajax.open("post", "modifyStudent.do");
            ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
            ajax.send("sno="+sno+"&name="+name+"&password="+password+"&sex="+sex+"&grade="+grade+"&type="+type+"&academicName="+academicName+"&majorName="+majorName+"&classesName="+classesName+"&dormitory="+dormitory+"&origin="+origin+"&address="+address+"&phone="+phone+"&status="+status);
        }
    ...
    <table class="table_context" width="100%">
        ...
        <tr>
            ...
            <td width="10%" height="50" align="left"><select name="sexop" id="sexop" style="height: 25px ;width: 200px"  >
                <option selected disabled hidden value="<%=student.getSex()%>" ><%=student.getSex()%></option>
        ...
        <tr>
            ...
            <td width="10%" height="50" align="left"><select name="gradeop" id="gradeop" style="height: 25px ;width: 200px"  >
                <option selected disabled hidden value="<%=student.getGrade()%>"><%=student.getGrade()%></option>
            </select></td>
        </tr>
        <tr>
            ...
            <td width="10%" height="50" align="left"><select name="typeop" id="typeop" style="height: 25px ;width: 200px"  >
                <option selected disabled hidden value="<%=student.getType()%>"><%=student.getType()%></option>
        ...
        <tr>
            ...
            <td width="10%" height="50" align="left"><select name="academic" id="academic" style="height: 25px ;width: 200px" onchange="majorAjax()" >
                <option selected disabled hidden value="<%=student.getAcademicName()%>"><%=student.getAcademicName()%></option>
            </select></td>
        </tr>
        <tr>
            ...
            <td width="10%" height="50" align="left"><select name="major" id="major" style="height: 25px ;width: 200px" onchange="classesAjax()">
                <option selected disabled hidden value="<%=student.getMajorName()%>" ><%=student.getMajorName()%></option>
            </select></td>
        </tr>
        <tr>
            ...
            <td width="10%" height="50" align="left"><select name="classes"  id="classes" style="height: 25px ;width: 200px" >
                <option selected disabled hidden value="<%=student.getClassesName()%>"><%=student.getClassesName()%></option>
            </select></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td width="10%" height="50" align="left"> <select id="statusop" name="statusop" lay-verify="" >
                <option value="<%=student.getStatus()%>" disabled selected hidden><%=student.getStatus().equals("1")?"使用中":"禁用中"%></option>
                <option value="1">启用</option>
                <option value="0">禁用</option>
            </select></td>
        </tr>
        <tr>
            <td height="50" colspan="2" align="center"><button class="layui-btn" onclick="modifyStudent('<%=student.getSno()%>')" >保存数据</button><button class="layui-btn layui-btn-danger" onclick="reset()">取消编辑</button></td>
        </tr>
    </table>
</body>
</html>
