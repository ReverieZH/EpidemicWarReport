	...
	<body style=" overflow-x:hidden;"  class="inner-container" onload="academicAjax()">
        ...
        <div style="margin: 10px 0px;">
            ...
            <c:forEach items="${questionnaires}" var="questionnaire" varStatus="status">
                <a href="account.do?order=${status.index}&questionnaireId=${questionnaire.questionnaireId}&taskId=${taskId}" class="layui-btn layui-btn-primary">${questionnaire.queName}</a>
            </c:forEach>
        </div>
            ...
            <%
                ...
                int order= Integer.parseInt(String.valueOf(request.getAttribute("order")));
                Questionnaire questionnaire=questionnaires.get(order);
                ...
                <c:choose>
                    <c:when test="${condition!=null}">
                        <c:choose >
                            <c:when test="${condition=='1'}" >
                                <c:if test="${fn:length(student.answers)>0}">
                                        <tr>
                                        ...
                                        <td width="10%" height="20" align="center">
                                          <button class="layui-btn layui-btn-normal">已打卡</button>
                                        </td>
                                        </tr>
                                </c:if>
                            </c:when>
                            <c:otherwise >
                                <c:if test="${student.answers==null||fn:length(student.answers)==0}">
                                <tr>
                                    <td width="10%" height="20" align="center">${student.name}</td>
                                    <c:choose >
                                        ...
                                        <c:otherwise>
                                            <c:forEach begin="0" end="${fn:length(questions)-1}">
                                                <td width="10%" height="20" align="center"></td>
                                            </c:forEach>
                                        </c:otherwise>
                                    </c:choose>
                                    <td width="10%" height="20" align="center">
                                        <button class="layui-btn ">补卡</button>
                                    </td>
                                </tr>
                                </c:if>
                            </c:otherwise>
                    ...
                    <c:otherwise>
                        <tr>
                            <td width="10%" height="20" align="center">${student.name}</td>
                            <c:choose>
                                <c:when test="${student.answers==null||fn:length(student.answers)==0}">
                                    <c:choose >
                                        <c:when test="${fn:length(questions)==0}">
                                        </c:when>
                                        <c:otherwise>
