...
<head>
    ...
    <style>
    	...
    	<script>
                ...
                function changeStatus(applyId,Status) {
                    ajaxRequest(applyId,Status);
                }

                function ajaxRequest( applyId,status) {
                    var ajax;
                    ...
                    ajax.send("applyId="+applyId+"&status="+status);
                }
                ...
                function deleteapply(){
                    var checkbox = document.getElementsByName('applyId');
                    var page=document.getElementById("page").value;
                    var url="";
                        for (var i = 0; i < checkbox.length; i++) {
                            if (checkbox[i].checked) {
                                url+="applyId="+checkbox[i].value+"&";
                            }
                        }

                      url+="page="+page;
                    location.href="deleteApply.do?"+url;
                }

                function deleteOneApply(applyId){
                    deletajaxRequest(applyId);
                }

                function deletajaxRequest( applyId ) {
                    var ajax;
                    if(window.XMLHttpRequest){//火狐
                        ajax=new XMLHttpRequest();
                    }else if(window.ActiveXObject){//ie
	...
	<body style=" overflow-x:hidden;"  class="inner-container">
		<div style="color: #666;">
            <a style="margin-top: 5px;" href="javascript:history.back()" ><i class="layui-icon layui-icon-next" style="color: #666;"></i><cite>门户应用类型</cite></a>
            <a style="width: 10px;height: 10px" href="javascript:location.reload()"><i class="layui-icon layui-icon-refresh"></i> </a>
            <button style="float: right;" class="layui-btn layui-btn-primary" onclick="deleteapply()">删除任务</button>
		<button style="float: right;" class="layui-btn layui-btn-primary" onclick="getiframe('getaddApply.do')">添加任务</button>
		...
		<table class="layui-table" style="width: 1050px;" lay-skin="line">
		             <tr>
                        ...
                        <td width="10%" height="20" align="center" >应用信息</td>
            ...
            <%
                Integer i= 0;
                PageBean<Apply> pageBean= (PageBean<Apply>) request.getAttribute("pageBean");
            %>
            ...
            <c:forEach begin="0" end="${pageBean.pageSize}" items="${pageBean.list}" var="task">
                <c:if test="${i<pageBean.totalRecord}">
                    <tr>
                        ...
                        <td width="2%" height="50" align="center"><input  type="checkbox" name="applyId" value="<%=pageBean.getList().get(i).getApplyId()%>"></td>
                        ...
                        <%}%>
                        <td width="12%" height="50" align="center"><button class="layui-btn" onclick="getiframe('getmodifyApply.do?applyId=<%=pageBean.getList().get(i).getApplyId()%>')">编辑</button> <button class="layui-btn layui-btn-warm" onclick="changeStatus('<%=pageBean.getList().get(i).getApplyId()%>','0')" >禁用</button> <button class="layui-btn layui-btn-danger" onclick="deleteOneApply('<%=pageBean.getList().get(i).getApplyId()%>')">删除</button></td>
                    </tr>
                </c:if>
