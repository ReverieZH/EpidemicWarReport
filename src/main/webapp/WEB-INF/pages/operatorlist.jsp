...
<html>
	<head>
    	...
    	<script>
                ...
                function ajaxRequest( majorId,status) {
                    ...
                    ajax.send("username="+username+"&status="+status);
                }
                ...
                function deleteajaxRequest(url) {
                    ...
                    ajax.open("get", "deleteOperator.do?"+url);
                    ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
                    ajax.send();
                }

                function deleteOneOperator(username){
                    deletajax(username);
                }

                function deletajax(username ) {
                    var ajax;
                    if(window.XMLHttpRequest){//火狐
                        ajax=new XMLHttpRequest();
                    }else if(window.ActiveXObject){//ie
                ...
                function checkAll() {
                    ...
                    var checkbox = document.getElementsByName('classesId');
                    if(checkALl.checked) {
                        for (var i = 0; i < checkbox.length; i++) {
                            if (!checkbox[i].checked) {
	...
	<body style=" overflow-x:hidden;"  class="inner-container">
		<div style="color: #666;">
            <a style="margin-top: 5px;" href="javascript:history.back()" ><i class="layui-icon layui-icon-next" style="color: #666;"></i><cite>教师数据管理</cite></a>
            <a style="width: 10px;height: 10px" href="javascript:location.reload()"><i class="layui-icon layui-icon-refresh"></i> </a>
            <button style="float: right;" class="layui-btn layui-btn-primary" onclick="deleleOperator()">删除教师</button>
		<button style="float: right;" class="layui-btn layui-btn-primary" onclick="getiframe('getaddOperator.do')">添加教师</button>
		
		<table class="layui-table" style="width: 1050px;" lay-skin="line">
            <thead>
		             <tr>
                        <td width="6%" height="20" align="center" > <input type="checkbox" onchange="checkAll()" id="check_all_box">全选</td>
                        <td width="5%" height="20" align="center" >账号名</td>
                         <td width="5%" height="20" align="center" >密码</td>
                         <td width="5%" height="20" align="center" >角色</td>
                         <td width="7%" height="20" align="center" >姓名</td>
                         <td width="8%" height="20" align="center" >学院</td>
            ...
            <%
                Integer i= 0;
                PageBean<Operator> pageBean= (PageBean<Operator>) request.getAttribute("pageBean");
            %>
            ...
            <c:forEach begin="0" end="${pageBean.pageSize}" items="${pageBean.list}" var="task">
                <c:if test="${i<pageBean.totalRecord}">
                    <tr>
                        ...
                        <td width="6%" height="50" align="center"><input  type="checkbox" name="username" value="<%=pageBean.getList().get(i).getUsername()%>"></td>
                        <td width="5%" height="50" align="center">
                            <%=pageBean.getList().get(i).getUsername()%><br>
                        </td>
                        <td width="5%" height="50" align="center">
                            <%= pageBean.getList().get(i).getPassword()%><br>
                        </td>
                        <td width="5%" height="50" align="center">
                            <%= pageBean.getList().get(i).getRoleStr()%><br>
                        </td>
                        <td width="7%" height="50" align="center">
                            <%= pageBean.getList().get(i).getName()%><br>
                        </td>
                        <td width="8%" height="50" align="center">
                            <%= pageBean.getList().get(i).getAcademicName()%><br>
                        </td>
                        <%
                            if(pageBean.getList().get(i).getValidty().equals("1")){
                        %>
                        <td width="5%" height="50" align="center" style="color: green" >使用中</td>
                        <%}else{%>
                        <td width="5%" height="50" align="center" style="color: red">禁用中</td>
                        <%}%>
                        <td width="12%" height="50" align="center"><button class="layui-btn" onclick="getiframe('getmodifyOperator.do?username=<%=pageBean.getList().get(i).getUsername()%>')">编辑</button> <button class="layui-btn layui-btn-warm" onclick="changeStatus('<%=pageBean.getList().get(i).getUsername()%>','0')" >禁用</button> <button class="layui-btn layui-btn-danger" onclick="deleteOneOperator('<%=pageBean.getList().get(i).getUsername()%>')">删除</button></td>
                    </tr>
                </c:if>
