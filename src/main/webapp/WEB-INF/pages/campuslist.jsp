...
<%@ page import="com.qidian.domain.Campus" %>
...
<html>
	...
	<body style=" overflow-x:hidden;"  class="inner-container">
		<div style="color: #666;">
            <a style="margin-top: 5px;" href="javascript:history.back()" ><i class="layui-icon layui-icon-next" style="color: #666;"></i><cite>学校校区设置</cite></a>
            <a style="width: 10px;height: 10px" href="javascript:location.reload()"><i class="layui-icon layui-icon-refresh"></i> </a>
            <button style="float: right;" class="layui-btn layui-btn-primary" onclick="deletetask()">删除校区</button>
		<button style="float: right;" class="layui-btn layui-btn-primary" onclick="getiframe('getaddTask.do')">添加校区</button>
		...
		<table class="layui-table" style="width: 1050px;" lay-skin="line">
		             <tr>
                        ...
                        <td width="10%" height="20" align="center" >校区信息</td>
            ...
            <%
                Integer i= 0;
                PageBean<Campus> pageBean= (PageBean<Campus>) request.getAttribute("pageBean");
            %>
            ...
            <c:forEach begin="0" end="${pageBean.pageSize}" items="${pageBean.list}" var="task">
                <c:if test="${i<pageBean.totalRecord}">
                    <tr>
                        ...
                        <td width="2%" height="50" align="center"><input  type="checkbox" name="campusId" value="<%=pageBean.getList().get(i).getCampusId()%>"></td>
                        <td width="10%" height="50" align="center">
                            <%= pageBean.getList().get(i).getCampusName()%><br>
                        </td>
                        <%
                        ...
                        <%}%>
                        <td width="12%" height="50" align="center"><button class="layui-btn" onclick="getiframe('getmodifyTask.do?taskId=<%=pageBean.getList().get(i).getCampusId()%>')">编辑</button> <button class="layui-btn layui-btn-warm" onclick="changeStatus('<%=pageBean.getList().get(i).getCampusId()%>','0')" >禁用</button> <button class="layui-btn layui-btn-danger" onclick="deleteOneTask('<%=pageBean.getList().get(i).getCampusId()%>')">删除</button><a href="../questionnarie/main.do?taskId=<%=pageBean.getList().get(i).getCampusId()%>" class="layui-btn layui-btn-normal">表单管理</a></td>
                    </tr>
                </c:if>
