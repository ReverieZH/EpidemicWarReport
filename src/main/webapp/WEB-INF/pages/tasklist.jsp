...
<html>
	<head>
    	...
    	<script>
                ...
                function changeStatus(taskId,Status) {
                    ajaxRequest(taskId,Status);
                }

                function ajaxRequest( taskId,status) {
                    var ajax;
                    ...
                    ajax.send("taskId="+taskId+"&status="+status);
                }
                ...
                function deleteOneTask(taskId){
                    deletajaxRequest(taskId);
                }

                function deletajaxRequest( taskId ) {
                    var ajax;
                    if(window.XMLHttpRequest){//火狐
                        ajax=new XMLHttpRequest();
                    }else if(window.ActiveXObject){//ie
	...
	<body style=" overflow-x:hidden;"  class="inner-container">
		<div style="color: #666;">
            ...
            <button style="float: right;" class="layui-btn layui-btn-primary" onclick="deletetask()">删除任务</button>
		<button style="float: right;" class="layui-btn layui-btn-primary" onclick="getiframe('getaddTask.do')">添加任务</button>
		...
		<table class="layui-table" style="width: 1050px;" lay-skin="line">
            ...
            <%
                Integer i= 0;
                PageBean<Task> pageBean= (PageBean<Task>) request.getAttribute("pageBean");
            %>
            ...
            <c:forEach begin="0" end="${pageBean.pageSize}" items="${pageBean.list}" var="task">
                <c:if test="${i<pageBean.totalRecord}">
                    <tr>
                        ...
                        <td width="2%" height="50" align="center"><input  type="checkbox" name="taskId" value="<%=pageBean.getList().get(i).getTaskId()%>"></td>
                        <td width="10%" height="50" align="center">
                            <%= pageBean.getList().get(i).getTaskName()%><br>
                            <span style="color: #cccccc">描述:<%=pageBean.getList().get(i).getTaskDesc()%></span>
                        </td>
                        <%
                        ...
                        <%}%>
                        <td width="12%" height="50" align="center"><button class="layui-btn" onclick="getiframe('getmodifyTask.do?taskId=<%=pageBean.getList().get(i).getTaskId()%>')">编辑</button> <button class="layui-btn layui-btn-warm" onclick="changeStatus('<%=pageBean.getList().get(i).getTaskId()%>','0')" >禁用</button> <button class="layui-btn layui-btn-danger" onclick="deleteOneTask('<%=pageBean.getList().get(i).getTaskId()%>')">删除</button><a href="../questionnarie/main.do?taskId=<%=pageBean.getList().get(i).getTaskId()%>" class="layui-btn layui-btn-normal">表单管理</a></td>
                    </tr>
                </c:if>
