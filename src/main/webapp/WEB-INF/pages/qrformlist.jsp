...
<html>
	<head>
    	...
    	<script>
                ...
                function changeStatus(qrformId,Status) {
                    ajaxRequest(qrformId,Status);
                }

                function ajaxRequest( qrformId,status) {
                    var ajax;
                    ...
                    ajax.send("qrformId="+qrformId+"&status="+status);
                }
                ...
                function deleteQrForm(){
                    var checkbox = document.getElementsByName('qrformId');
                    var page=document.getElementById("page").value;
                    var url="";
                        for (var i = 0; i < checkbox.length; i++) {
                            if (checkbox[i].checked) {
                                url+="qrformId="+checkbox[i].value+"&";
                            }
                        }
                ...
                function deleteajaxRequest(url) {
                    ...
                    ajax.open("get", "deleteQrForm.do?"+url);
                    ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
                    ajax.send();
                }


                function deleteOneQrForm(qrformId){
                    deletajax(qrformId);
                }

                function deletajax( qrformId ) {
                    var ajax;
                    if(window.XMLHttpRequest){//火狐
                        ajax=new XMLHttpRequest();
                    }else if(window.ActiveXObject){//ie
	...
	<body style=" overflow-x:hidden;"  class="inner-container">
		<div style="color: #666;">
            <a style="margin-top: 5px;" href="javascript:history.back()" ><i class="layui-icon layui-icon-next" style="color: #666;"></i><cite>签到任务设置</cite></a>
            <a style="width: 10px;height: 10px" href="javascript:location.reload()"><i class="layui-icon layui-icon-refresh"></i> </a>
            <button style="float: right;" class="layui-btn layui-btn-primary" onclick="deleteQrForm()">删除任务</button>
		<button style="float: right;" class="layui-btn layui-btn-primary" onclick="getiframe('getaddQrForm.do')">添加任务</button>
		...
		<table class="layui-table" style="width: 1050px;" lay-skin="line">
		             <tr>
                        ...
                        <td width="10%" height="20" align="center" >任务信息</td>
            ...
            <%
                Integer i= 0;
                PageBean<QRForm> pageBean= (PageBean<QRForm>) request.getAttribute("pageBean");
            %>
            ...
            <c:forEach begin="0" end="${pageBean.pageSize}" items="${pageBean.list}" var="task">
                <c:if test="${i<pageBean.totalRecord}">
                    <tr>
                        ...
                        <td width="2%" height="50" align="center"><input  type="checkbox" name="qrformId" value="<%=pageBean.getList().get(i).getQrformId()%>"></td>
                        <td width="10%" height="50" align="center">
                            <%= pageBean.getList().get(i).getQrformName()%><br>
                            <span style="color: #cccccc">描述:<%=pageBean.getList().get(i).getQrformDesc()%></span>
                        </td>
                        <%
                        ...
                        <%}%>
                        <td width="12%" height="50" align="center"><button class="layui-btn" onclick="getiframe('getmodifyQrForm.do?qrformId=<%=pageBean.getList().get(i).getQrformId()%>')">编辑</button> <button class="layui-btn layui-btn-warm" onclick="changeStatus('<%=pageBean.getList().get(i).getQrformId()%>','0')" >禁用</button> <button class="layui-btn layui-btn-danger" onclick="deleteOneQrForm('<%=pageBean.getList().get(i).getQrformId()%>')">删除</button><a href="../data/qraccount.do?questionnaireId=<%=pageBean.getList().get(i).getQuestionnaireId()%>" class="layui-btn layui-btn-normal">签到数据</a><button class="layui-btn" onclick="getiframe('getQrCode.do?questionnaireId=<%=pageBean.getList().get(i).getQuestionnaireId()%>')">码图</button></td>
                    </tr>
                </c:if>
